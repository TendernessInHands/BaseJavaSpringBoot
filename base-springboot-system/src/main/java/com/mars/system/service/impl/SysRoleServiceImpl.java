...
public class SysRoleServiceImpl implements ISysRoleService {
    ...
    public Set<String> selectRolePermissionByUserId(String userId) {
        ...
        for (SysRole perm : perms) {
            if (LocalStringUtils.isNotNull(perm)) {
    ...
    public String checkRoleNameUnique(SysRole role) {
        String roleId = LocalStringUtils.isNull(role.getRoleId()) ? "" : role.getRoleId();
        SysRole info = roleMapper.checkRoleNameUnique(role.getRoleName());
        if (LocalStringUtils.isNotNull(info) && !info.getRoleId().equals(roleId)) {
    ...
    public void checkRoleAllowed(SysRole role) {
        if (LocalStringUtils.isNotNull(role.getRoleId()) && role.isAdmin()) {
    ...
    public int insertRole(SysRole role) {
        // 新增角色信息
        role.setRoleId(SnowflakeIdWorker.getSnowId());
        role.setOrgCode(tokenService.getOrgCode(ServletUtils.getRequest()));
